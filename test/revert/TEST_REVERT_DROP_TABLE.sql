...
BEGIN
  ...
  -- ASSERT tab_log_id = test_log_id, format('Error: Expected log_id %s got %s', test_log_id, tab_log_id);
  ASSERT lower(tid_range) IS NOT NULL, format('Error: Lower transaction id %s does not match the id %s of DROP TABLE event', upper(tid_range[1]), test_transaction);
END;
$$
LANGUAGE plpgsql;
